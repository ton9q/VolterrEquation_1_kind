...
uses SysUtils, Variants, Math, Chart, Grids, WordXP, ExcelXP, ComObj, Classes;
...
type
  ...
  private
    ...
    myList:TStringList; // sheet for storing previous results
  ...
  public
    ...
    constructor Create(aa,bb,hh:extended);
    ...
    procedure updateN(); // update parametr N
    procedure displayOnGraph(str:TStringGrid;chr:TChart); // draw a graph of the results
    procedure FillInTheTable(str:TStringGrid); // fill in the table of results of desicions
    // work with MS Office and Excel
...
procedure TIntegral.updateN();
begin
  n:=trunc((b-a)/h+1);
end;

procedure TIntegral.displayOnGraph(str:TStringGrid;chr:TChart);
var
  int:integer;
begin
  chr.Series[0].Clear;
...
procedure TIntegral.FillInTheTable(str:TStringGrid);
var
  i:integer;
  x:extended;
  buf:extended;
...
function TIntegral.yy(i:integer):extended;
var
  g:extended;
  j:integer;
  x,k1:extended;
...
// K(x, s) = 2 + x^2 - s^2
function TIntegral.k(x,s:extended):extended;
begin
  result:=2+(x-s)*(x+s);
end;

// F(x) = x^2
function TIntegral.f(x:extended):extended;
begin
  result:=power(x,2);
end;

// y = x * e^(-x^2/2)
function TIntegral.y(x:extended):extended;
begin
  result:=x*exp(-power(x,2)/2);
end;

constructor TIntegral.Create(aa,bb,hh:extended);
begin
  a:=aa;
  b:=bb;
  h:=hh;
...
function TIntegral.GetA():extended;
begin
  result:=a;
end;

function TIntegral.GetB():extended;
begin
  result:=b;
end;

function TIntegral.GetH():extended;
begin
  result:=h;
end;

function TIntegral.GetN():integer;
begin
  result:=n;
end;

procedure TIntegral.SetA(aa:extended);
begin
  a:=aa;
  updateN();
end;

procedure TIntegral.SetB(bb:extended);
begin
  b:=bb;
  updateN();
end;

procedure TIntegral.SetH(hh:extended);
begin
  h:=hh;
  updateN();
end;

procedure TIntegral.TextWord(word:TWordDocument;appl:TWordApplication);
begin
...
procedure TIntegral.TextWord(word:TWordDocument;appl:TWordApplication;str:TStringGrid; chr:TChart);
var
  int:integer;
  end1:OleVariant;
begin
...
procedure TIntegral.TextExcel(str:TStringGrid;chr:TChart);
var
  int:integer;
  Excel,Work:variant;
begin
...
procedure TIntegral.writeTextFile(filename:string);
var
  f:TextFile;
begin
...
procedure TIntegral.writeTextFile(filename:string;str:TStringGrid);
var
  f:TextFile;
  int:integer;
begin
...
procedure TIntegral.readTextFile(filename:string);
var
  f:TextFile;
begin
  AssignFile(f,filename);// associate a file variable with the file name
